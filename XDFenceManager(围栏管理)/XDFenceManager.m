...
- (void)postFenceDicSuccess:(void (^)(NSDictionary *success))successBlock
    ...
    if (_allFenceDataArray.count!=0) {
        successBlock(nil);
    }
    else{
        _allFenceDataArray = [[NSMutableArray alloc] init];
        [[XDNetWork sharedInstance]postRequestWithUrl:url andParameters:nil success:^(NSDictionary *success) {
            ...
            for (int a=0; a<listDeviceArray.count; a++) {
                ...
                [_allFenceDataArray addObject:model];
            }
            successBlock(nil);
...
- (NSMutableArray *)allFenceDataArray{
    NSLog(@"----_allFenceDic---%@",_allFenceDataArray);
    return _allFenceDataArray;
}
/**
...
- (NSMutableArray *)fenceShapeArray{
    if (_fenceShapeArray==nil) {
        _fenceShapeArray = [[NSMutableArray alloc] init];
    }
    return _fenceShapeArray;
}
@end
