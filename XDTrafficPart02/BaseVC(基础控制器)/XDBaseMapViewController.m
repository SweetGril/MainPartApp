...
- (void)creatAnnotationPoint{
    if (_allDeviceDictionary.allKeys.count>0) {
        ...
        [_bgMapView removeAnnotations:[XDDeviceManager sharedManager].allDeviceAnnotationArray];
        [[XDDeviceManager sharedManager].allDeviceAnnotationArray removeAllObjects];
...
- (void)requestFenceData{
    [[XDFenceManager sharedManager]postFenceDicSuccess:^(NSDictionary *success) {
        [_bgMapView removeOverlays:[XDFenceManager sharedManager].fenceShapeArray];
        [[XDFenceManager sharedManager].fenceShapeArray removeAllObjects];
        for (int a=0; a<[XDFenceManager sharedManager].allFenceDataArray.count; a++) {
            XDCricleModel *model = [XDFenceManager sharedManager].allFenceDataArray[a];
            if ([model.shape isEqualToString:@"circle"]==YES) {
                ...
                [[XDFenceManager sharedManager].fenceShapeArray addObject:circle];
            }
...
- (void)createBrokenLineWithArray:(NSArray *)dataParkLaneArr andStstus:(NSString *)status andName:(NSString *)scopeName andIndex:(int)indexNum andSpenceID:(NSString *)spenceID{
    ...
    [[XDFenceManager sharedManager].fenceShapeArray addObject:commonPoly];
